...
> These are reconstructions from the rendered PNGs (since draw.io macro contents don't export through the Confluence markdown API). Verify against the originals (see `kivira-diagrams-index.md` for direct links) when accuracy matters for ADRs / sign-off.
...
C4 Level 1. Kivira as central system + 4 persons + 4 external systems.
...
flowchart TB
    ...
    KA["<b>Kivira Admin</b><br/><i>Person</i>"]:::person
    CL["<b>Clinician</b><br/><i>Person</i>"]:::person

    K["<b>Kivira</b><br/><i>Software System</i><br/>The entire system with all components"]:::system
    ...
    KA -- "Browser · Kivira Admin Web App" --> K
...
C4 Level 2. Color legend: dark-blue = Person, blue = Andersen-implemented container, purple = Kivira-implemented, green = AWS managed, grey = external.
...
flowchart TB
    ...
    P[Patient]:::person
    KA[Kivira Admin]:::person
    CA[Clinic Admin]:::person
    CL[Clinician]:::person

    subgraph Kivira["Kivira [System]"]
        direction TB
        PMA["Patient Mobile App<br/>iOS · React Native · TS"]:::andersen
        PMB["Patient Mobile App Backend<br/>Python · FastAPI"]:::andersen
        KAW["Kivira Admin Web App<br/>Python · Django Admin"]:::andersen
        CAW["Clinic Admin Web App<br/>Python · Django Admin"]:::andersen

        KG["Kivira Games<br/>React Native lib<br/>(cognitive games)"]:::kivira
        KR["Kivira Recommendation<br/>Python lib · Rules Engine"]:::kivira
        KML["Kivira ML Environment<br/>SageMaker"]:::kivira
...
flowchart TB
    ...
    subgraph KPM["Kivira Patient Mobile App · React Native"]
        SJL[SurveyJS library<br/>npm package<br/>WebView for HTML/JS]
    end

    subgraph KAA["Kivira Admin App"]
