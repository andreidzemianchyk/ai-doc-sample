...
| --- | --- |
| `kivira-initial-phase-deliverables.md` | Full text snapshot of all 54 Confluence pages under *Initial Phase Deliverables* (parent 416446964) — AVD sections, Vision&Scope, Sprint-1 epics with US scenarios and ACs. The substrate of record. |
| `kivira-diagrams-index.md` | Direct download URLs for the 9 active draw.io PNGs + `.mxfile` sources + textual descriptions. Use when the doc-gen session needs the original visual reference. |
| `kivira-diagrams-mermaid.md` | All 9 diagrams reconstructed in Mermaid (1 sequence, 8 flowcharts). Paste into target docs as-is. |
...
- **Kivira** — product owner, supplying cognitive games (RN library), recommendation Python rules engine, Kivira Admin Web App (out of Andersen scope), and clinical questionnaire JSON.
...
- **Clinician Report** — generated server-side from intake + screener + games. Three FHIR resources sent to EPIC: Observation (mandatory, triggers PGHD inbound queue notification), Condition, DocumentReference (points to PDF held on Kivira side; SMART FHIR auth-gated download by clinician).
- **Kivira Admin App** — explicitly out of Andersen delivery scope (future Kivira work).

**Architecture in one diagram (C4 L2).** Patient Mobile App + Backend + Kivira Admin Web App + Clinic Admin Web App, all behind AWS ALB; data layer in RDS Postgres (Multi-AZ) + ElastiCache + S3; managed services CloudWatch + SNS/SES/EventBridge + Cognito for non-EPIC auth. Kivira-implemented libraries (Games, Recommendation, ML Environment on SageMaker) embed into the apps. SurveyJS as external library. See `kivira-diagrams-mermaid.md` §3.
...
The doc-gen session must demonstrate the following methodology beats. Adaptations for Kivira's presale substrate are noted in italic.
...
2. **Wide-table schema doc via iterative column-classification** (TA §5.1).
   *Adapt:* Kivira has no committed code yet. Apply the column-classification methodology to the **Patient Profile composite** (demographics + clinical fields + FDB-coded fields specified in Epic 2 Feature 1 + Feature 3 + Feature 4 of the Mobile App). Treat the spec/Confluence pages as the source of truth in place of code. `VALIDATE:` flags work the same way — they call out where the source pages disagree, where a field is mentioned but not specified, or where downstream consumption is unclear.
3. **Per-module documentation** (TA §3 structure; §5.5 for business-rule catalog).
   *Adapt:* Best Kivira "module" candidate for the sample is **Mobile App · Authentication / Authorization** (Epic-1 Mobile, 2 features, 9 user stories with scenarios and acceptance criteria). Rich enough for business-rule catalog with stable IDs. Alternative: Epic-2 Intake & Screener (heavier on FHIR/FDB integration).
4. **AI-native entry points + repo-root conventions** (TA §6 for AI entries; TA §3 for root files).
   Root `CLAUDE.md` + root `AGENTS.md` (cross-tool overlay; CLAUDE.md @-imports it per TA §6 — single source of truth). Module-level `CLAUDE.md` per module. Root `CONVENTIONS.md` (annotation legend + validate-flag syntax + doc structure rules — required by TA §3 / §8 week-1 deliverable). Root `GLOSSARY.md` (domain terminology — Kivira already has 24+ acronyms enumerated in AVD §1.3, directly transposable). `llms.txt` is optional in TA §6 ("preferred default where toolchain supports it cleanly") — mention in CLAUDE.md and defer authoring to a follow-up.
5. **MADR Architecture Decision Records** (TA §2).
   *Adapt:* ADRs are *retrospective* for SourceWare. For Kivira-presale they are *retrospective on discovery-phase decisions* — pick a decision visible in the AVD (D-codes) and recover the rationale from the Confluence content. Best candidates listed in §5.4 below.
6. **Explicit uncertainty handling** (TA §4 Stage 3).
   `> [!validate]` callouts inline, `<!-- VALIDATE: ... -->` comments, `## Open questions` section per doc. Plenty of natural candidates in Kivira docs — many "TBD" strings explicit in user stories.
...
- One-paragraph framing: demonstration work on Kivira (Andersen presale, healthcare/precision-psychiatry), not redacted client work.
...
- *Read these before doing anything substantive*: `docs/architecture/overview.md`, `docs/schema/tables/patient-profile.md`, `CONVENTIONS.md` (if added), `GLOSSARY.md` (if added — Kivira has a glossary in AVD §1.3).
- *Task routing*:
...
**Why it exists.** TA §3 root-level file. Kivira's AVD §1.3 already enumerates 24+ acronyms — directly transposable, no invention.
...
- Term table (alphabetised). Direct copy from AVD §1.3 (`kivira-initial-phase-deliverables.md` §3.1.3 in the consolidated file) of: ADHD, AWS, CDSS, CIDI, CPT Codes, D.O.B., DSM, EHR, EPIC, ERT, FDA, FDB, FHIR, GAD, HIPAA, JSPsych, MAUs, MCI, MDD, ML, MVP, OCD, PHI, PTSD, SLA, TBI, WCST.
- Plus 5–8 Kivira-specific terms inferred from the docs but not in the AVD glossary (with `> [!validate]` if the spec is silent on the precise meaning):
  - **App Orchard** — EPIC's marketplace for third-party SMART on FHIR apps; registration is the gating requirement for Kivira's invite-link plugin (ADR-0001 dependency).
  ...
  - **SMART on FHIR** — the OAuth2-based framework Kivira uses to embed in EPIC and authenticate via MyChart.
  ...
  - **Hyperspace / Hyperdrive** — EPIC's desktop and web client environments; the Kivira plugin must function in both.
...
1. **Scope** — what the docs cover (Mobile App, Clinic Web App, Clinician Report, AWS infrastructure, EPIC/FDB/Stripe/Intercom integrations) and what they don't (Kivira Admin App, ML training in MVP, Android, non-EPIC EHRs, FDA Class II+ functionality).
...
8. **System Context view (C4 L1)** — paste Mermaid block from `kivira-diagrams-mermaid.md` §2.
9. **Container view (C4 L2)** — paste Mermaid block from `kivira-diagrams-mermaid.md` §3.
10. **Deployment view** — paste Mermaid block from `kivira-diagrams-mermaid.md` §5 (MVP deployment). The "final product" version (§4) can be referenced as "post-MVP".
...
- Whether the Andersen team's Kivira Admin App handoff is documentation-only or includes a runbook.

### 5.3a `sample-work/docs/architecture/integration-points.md` (1.5 pages)

**Why it exists.** TA §3 lists `architecture/integration-points.md` ("External systems, APIs, file-based exchanges"). Kivira's integration surface is broad enough that consolidating it in one place — instead of scattering across `overview.md` and the data-flow pages — pays dividends for both human and agent readers.
...
- **SurveyJS** — external library bundled into the patient mobile app (npm package + WebView for HTML/JS) and the Kivira Admin App (SurveyJS Creator embedded). License: MIT for runtime; Creator commercial for closed-source authoring — flag with `> [!validate]` if the licensing path is unclear.
...
**Why it exists.** TA §3 prescribes `architecture/data-flows/` as "one file per high-stakes flow". For Kivira the two highest-stakes flows are patient onboarding and report-to-clinician (this file + 5.3c).

**Content.**

- **Trigger.** Clinician generates invite via EPIC Custom Action (or non-EPIC admin page). Patient receives link + backup code.
- **Mermaid block.** Embed `kivira-diagrams-mermaid.md` §7 (the full `login` flowchart).
...
- **Mermaid block.** Embed `kivira-diagrams-mermaid.md` §6 (the EPIC integration sequence).
- **Step-by-step prose.**
  ...
  2. Recommendation library invoked (Kivira's black-box rules engine, D30); generates ranked diagnoses + treatment options + CPT codes.
  3. PDF generated from HTML template (D8). Stored on Kivira side. URL produced.
  ...
  7. Clinician finalises diagnosis in EPIC. On next patient login, Kivira refreshes patient profile and pulls the final diagnosis back.
- **Critical invariants.**
...
- **`## Open questions`** (2–3). Suggested: how the PDF link is rotated / expired; whether DocumentReference type coding follows a Kivira convention or EPIC default; whether the clinician-finalised diagnosis pulled back is stored as a separate Condition or merged into the existing patient_profile.

### 5.3d `sample-work/docs/architecture/release-coexistence.md` (1 page)

**Why it exists.** Stand-in for TA §3 `architecture/legacy-2.0-coexistence.md`. Kivira has no legacy stack, so the cross-cutting coexistence story is between **MVP, 2nd release, and 3rd release** — features that ship in parallel during MVP+1 and MVP+2 while older patient cohorts stay on earlier feature sets.
...
- **Context.** Each US primary-care clinic that licenses Kivira runs its own EPIC EHR + MyChart instance. The patient onboarding flow must authenticate the patient against the *correct* MyChart, link the resulting tokens to the patient's record in Kivira, and do so without a custom Kivira account/password layer (to keep the auth surface minimal and HIPAA-friendly).
- **Decision.** Use MyChart as the SSO provider, per-clinic. The Kivira Patient Mobile App opens an embedded MyChart login page whose URL is generated by the Kivira backend after invite-link / backup-code validation determines the patient's clinic. Authentication uses SMART on FHIR OAuth2 with PKCE. Access + refresh tokens are stored encrypted on the device (D9) and on the backend; subsequent logins re-validate or refresh via the backend.
...
- **Consequences.**
  - *Positive:* No Kivira-managed credentials for patients → reduced PHI exposure surface; clinician workflow stays inside EPIC; alignment with App Orchard certification path.
...
- **Scope.** What's in the schema layer (Kivira-internal tables: patient_profile, consents, intake_responses, game_results, reports, follow_up_attempts, subscriptions, invites, etc. — only `patient-profile.md` is authored in this slice; the rest are listed as "to be authored").
...
**Why it exists.** TA §3 / TA §5.2 — implicit FK catalog. Even pre-code, Kivira's spec has several cross-resource relationships that aren't (yet) declared as FK constraints but are load-bearing.

**Content.**

- One-paragraph framing: catalog of relationships that *aren't* declared FK at the database level — either because they cross the system boundary (Kivira ↔ EPIC), or because the implementation is pending.
...
| `intake_responses.patient_id` | `patient_profile.patient_id` | N:1 | Epic-2 F2 store interim+final results | Resolved (Kivira-internal). |
| `reports.patient_id` | `patient_profile.patient_id` | N:1 | Epic-3 F1 report assembly | Resolved. |
| `reports.documentreference_url` | external (Kivira-hosted file storage) | 1:1 | D31 + AVD 4.4 | Resolved (cross-system). |
...
- **Cross-system relationships callout.** EPIC FHIR resource references are not FK constraints inside Kivira's DB but must be treated as load-bearing — patient_profile changes that drop `epic_patient_id` would break the report-to-clinician flow.
...
- **Logical groupings** (8–12 clusters; pull from the specs):
  1. *Identity / linkage* — patient_id (Kivira-internal UUID), epic_patient_id, mychart_token_ref.
...
| --- | --- | --- | --- | --- | --- |
| patient_id | uuid | no | Kivira-internal stable patient identifier. | Referenced from every other Kivira table (intake_responses, game_results, reports). | Resolved. |
...
| pregnancy_status | enum | yes | Used by FDB DDC screening (Drug-Disease Contraindications) and DPT. | US-3.5 lists as one of the patient-profile inputs. | `VALIDATE:` Values not enumerated in the spec (pregnant / not-pregnant / unknown / not-applicable). Affects FDB query input shape — workshop topic with Kivira clinical team. |
...
- What is the canonical FDB version used by Kivira at MVP launch? Affects code mapping stability.
...
- Is the Kivira-internal patient_id derived from `epic_patient_id` (deterministic) or independent (UUID v4)? Affects edge cases when a patient changes clinics.
...
- *Integration points.* MyChart (per-clinic OAuth2 + PKCE), Cognito (non-EPIC fallback), Kivira backend (token storage + invite validation + consent capture), Kivira Patient Profile (first-login data fetch from EPIC).
...
- Embedded **flowchart Mermaid** — paste the login flow from `kivira-diagrams-mermaid.md` §7. This *is* the architectural anchor for the module.
...
3. *Known divergences.*
   - EPIC path triggers a patient-data prefill from EPIC FHIR (step 15b). Non-EPIC path skips this step — the Kivira side knows only what was captured at signup.
...
4. *Migration trajectory.* Non-EPIC support is positioned as "fallback / parallel path" in the AVD; the architectural intent is that MVP launches with one or more EPIC clinics first, with non-EPIC support exercised but not the primary path. `> [!validate]` This is the architectural read — the docs do not explicitly say "EPIC path is canonical, non-EPIC is fallback". Workshop topic with Kivira leadership.

---

## 6. Diagram inventory for the sample

The Mermaid blocks already produced in `kivira-diagrams-mermaid.md` map onto the sample as follows:
...
Every Mermaid block has been built faithfully from the rendered draw.io PNG (not from the `.mxfile` source). For ADR-grade accuracy the doc-gen session should cross-verify the auth-flow Mermaid against `kivira-diagrams-index.md` §7 description before publishing.
...
- Whether Andersen's handoff to Kivira on the Admin App includes a runbook.
...
| Criterion | How it's satisfied for Kivira sample |
| --- | --- |
| Mermaid diagrams render | Use blocks from `kivira-diagrams-mermaid.md` verbatim; render-test in MkDocs Material before delivery. |
| File paths / function names cited exist | Substrate is pre-code → cite Confluence page IDs and US numbers instead; spot-check ≥ 5 citations against `kivira-initial-phase-deliverables.md`. |
...
- The Kivira Admin App (explicitly out of Andersen scope per the source).
...
- `business-logic/` (pricing.md, commission.md, customer-forks.md, edge-cases.md) — SourceWare-specific. Kivira's business rules are captured per-module in `modules/auth-authorization/business-rules.md`, which is the appropriate granularity for the auth domain. A real Kivira engagement would add an analog under a different label (e.g., `clinical-logic/recommendation-rules.md`, `clinical-logic/screening-criteria.md`) — flag in the README.
...
2. Open `kivira-initial-phase-deliverables.md` for content lookups.
3. Open `kivira-diagrams-mermaid.md` for diagram blocks.
...
6. Self-review pass: render-test all Mermaid blocks, verify cross-references resolve, spot-check ≥ 5 Confluence-page / US-ID citations against `kivira-initial-phase-deliverables.md`.
